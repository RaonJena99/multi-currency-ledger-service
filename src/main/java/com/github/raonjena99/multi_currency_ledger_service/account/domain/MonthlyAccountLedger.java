...
public class MonthlyAccountLedger extends BaseEntity {
    ...
    @Column(name = "average_unit_price_currency", nullable = false, length = 10)
    ...
    public static MonthlyAccountLedger carryForwardFrom(MonthlyAccountLedger prev, String currentMonth) {
        ...
        ledger.baseCurrency = prev.getBaseCurrency();
     ...
     */
    public void addBalance(Money quantityToAdd, BigDecimal unitPrice) {
        ...
        // 신규 매입 가치 계산 = 신규 단가 * 추가 수량
        BigDecimal newAdditionValue = unitPrice.multiply(quantityToAdd.getAmount());

        // 매수한 수량만큼 잔고 업데이트
        this.balance = this.balance.add(quantityToAdd);

        // 새로운 총 가치 = 기존 총 가치 + 신규 매입 가치
        BigDecimal newTotalValue = totalCurrentValue.add(newAdditionValue);
        
        // 이동 평균법에 따른 새로운 평균 단가 갱신 = 총 가치 / 갱신된 총 수량
        this.averageUnitPrice = newTotalValue.divide(this.balance.getAmount(), 18, RoundingMode.HALF_EVEN);
    }
     ...
     */
    public BigDecimal subtractBalance(Money quantityToSubtract) {
